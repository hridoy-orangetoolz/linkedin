...
const trackData = async (userUid, data) => {
  ...
  if (author.split(":")[2] === "person")
    increaseObject["numPersonalPost"] = increaseValue;
  else if (author.split(":")[2] === "organization")
    increaseObject["numCompanyPost"] = increaseValue;
  else increaseObject["numOhterAuthorPost"] = increaseValue;

  if (shareMediaCategory === "ARTICLE")
    increaseObject["numArticlePost"] = increaseValue;
  else if (shareMediaCategory === "IMAGE")
    increaseObject["numImagePost"] = increaseValue;
...
const postOnLinkedin = (newPost) =>
  new Promise(async (resolve, reject) => {
    ...
    if (
      userUid === "linkedin:3A8ySOLYhe" ||
      process.env.GCLOUD_PROJECT === "social-scheduler-create-post"
    ) {
      const linkedinApi = new LinkedinApi();
      await linkedinApi.retrieveAccessToken(userUid);
      const res = await linkedinApi
        .request("LINKEDIN_API_ROLE_UGC_POST", { body })
        .catch((err) => reject(err));

      if (process.env.GCLOUD_PROJECT === "social-scheduler-create-post") {
        if (res !== undefined && res.id) {
          ...
          db.collection("user")
            ...
            .collection("posted")
            .doc(newPost.id)
            .set({ ...newPost.data(), ...{ urnPostId: res.id } });
          db.collection("user")
            .doc(userUid)
            .collection("post")
            .doc(newPost.id)
          ...
          if (media && media.fileInfo && media.fileInfo.filePath)
            bucket.file(media.fileInfo.filePath).delete();

          const mailProvider = new MailProvider();
          await mailProvider
            .sendPostConfirmation(newPost.data())
            .then(() => resolve("Sended"))
